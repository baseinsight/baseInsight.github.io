...
可在springsecurity-rest-db , springsecurity-rest-redis等插件中使用logout功能.

本插件可以与 FullRestController注解一起使用.
...
[source,groovy]
----
    implementation('io.github.baseinsight:plugin-springsecurity-rest-starter:x.x.x')
----
...
----
    ...
    implementation('io.github.baseinsight:plugin-springsecurity-rest-starter:x.x.x-SNAPSHOT')
----
...
登录功能的地址配置在application.yml中base.springsecurity.rest.login.endpointUrl,默认是/api/login ,可视项目情况修改为其他路径
[source,json]
----
...
现在我们使用token访问在requestmap中标明安全防护的rest接口,此处使用 /baseUser/json 示例 (配置为ROLE_ADMIN角色可访问)

TIP: 实际场景应该是api的接口地址,是使用FullRestController标注的rest接口
...
headers 上 增加X-Requested-With ,值为 XMLHttpRequest
...
系统在rest认证失败后增加response header:AuthenticationExceptionName和AuthenticationExceptionInfo (需要用URLDecode获取中文)
